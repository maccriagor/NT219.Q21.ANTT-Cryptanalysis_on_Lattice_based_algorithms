...
# (aarch64-linux-gnu toolchain)". Passes CMake a toolchain file and sets
# -DOQS_USE_OPENSSL=OFF for cross builds (no target-arch OpenSSL on the host).
# Output: $PQC_ROOT/liboqs-aarch64-cross  (copy to the ARM box to run).
# =============================================================================
# DESIGN NOTES
#   - Supply CMake a toolchain file + -DOQS_USE_OPENSSL=OFF (no ARM OpenSSL
#     exists on the x86 host to link against).
