...
case $choice in
    ...
    8) name="Hybrid_X25519_MLKEM768"; sig="p256_mldsa44"; kem="x25519_mlkem768" ;;
...
if [ ! -f "$CSV_OUT" ]; then
    echo "Suite,Min(ms),Mean(ms),P50(ms),P95(ms),P99(ms),Max(ms),Throughput_Conn_Sec" > "$CSV_OUT"
fi
...
echo "$name,$min,$mean,$p50,$p95,$p99,$max,$connections_per_sec" >> "$CSV_OUT"
